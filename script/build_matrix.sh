...
#         | awk '{for (i=1;i<=NF;i++) \
#                   if($i!=$i+0) printf "%s\t", $i; \
#                   else if($i==$i+0 && $i>1) printf "%.4f\t", 1; \
#   	    	       else if($i==$i+0 && $i<=1) printf "%.4f\t", $i; print ""}' \
#   	    	> temp;
        awk 'NR>1' tot-$IR$i-$REF-$TAR.$INF_FTYPE \
         | awk '{for (i=1;i<=NF;i++) \
         {print $1;continue;} \
                   if($i>1) printf "%.4f\t", 1; \
   	    	       else if($i<=1) printf "%.4f\t", $i; print ""}' \
   	    	> temp;        cat "${TAR}_HORIZ_PAD" > "temp-tot"
        cat "temp" >> "temp-tot"
