...
#            < $FLD_chromosomes/$c.$FILE_TYPE > temp;
#        cat temp | grep -v ">" > $HUMAN_CHR${CURR_CHR}_$x   # remove header line
        goose/src/goose-mutatefasta -s $x -a5 -mr $MRATE " " \
            < $FLD_chromosomes/$c.$FILE_TYPE \
            | grep -v ">" > $HUMAN_CHR${CURR_CHR}_$x    # remove header line
    done
done
#rm -f temp*
