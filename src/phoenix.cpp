...
int main (int argc, char *argv[])
{
    ...
    {
        //TODO: aya jaygozini vase sharte "i+j < n_models" has?
    ...
    */
    ...
    {
        FCM decModel;
        /// reference(s) address(es)
        for (string s : mixModel.getRefAddr())
            decModel.pushRefAddr(s);
        for (string s : mixModel.getTarAddr())
            decModel.pushTarAddr(s); /// tar. address(es)
