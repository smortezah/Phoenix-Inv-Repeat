...
REF_DATASET="";  for i in 17 18; do REF_DATASET+=$HUMAN_CHR${i}" "; done # reference dataset
TAR_DATASET="";  for i in 17 18 21; do TAR_DATASET+=$CHIMP_CHR${i}" "; done # target dataset
#TAR_DATASET="";  for i in 21; do TAR_DATASET+=$HUMAN_CHR${i}" "; done # reference dataset


### Chimpanzee as reference, Human as target
#REF_DATASET="";  for i in 24; do REF_DATASET+=$CHIMP_CHR${i}" "; done # reference dataset
#TAR_DATASET="";  for i in 22; do TAR_DATASET+=$HUMAN_CHR${i}" "; done # target dataset
...
for ir in $INV_REPEATS; do
 for alphaDen in $ALPHA_DENS; do
  for refDataset in $REF_DATASET; do
   echo -e "# ref\ttar\tir\talpha\tctx\tbpb\ttime(s)" \
